...
- 示例二：
    ...
    # output -> 150
    # reason -> [10,20,30,40,50] 是元素和最大的升序子数组，最大元素和为 150 。
    ```
...
- 示例 2：
    ...
    # input -> nums = [3,1,4,2]
    # output -> true
    # reason -> 序列中有 1 个 132 模式的子序列： [1, 4, 2] 。
    ```
...
- 示例 1：
    ...
    # input ->  [1,1,1,2,2,3]
    # output -> 5
    # reason -> 函数应返回新长度 length = 5, 并且原数组的前五个元素被修改为 1, 1, 2, 2, 3 。 不需要考虑数组中超出新长度后面的元素。
    ```
...
- 示例 2：
    ...
    # output -> 7
    # reason -> 函数应返回新长度 length = 7, 并且原数组的前五个元素被修改为 0, 0, 1, 1, 2, 3, 3 。 不需要考虑数组中超出新长度后面的元素。
    ```
...
- 思路：
    ...
    这题目很难，使用动态规划能够比较直观的去处理各种情况，但是理解动态转移方程的过程会比较难。

    这里直接引用 [LeetCode](https://leetcode-cn.com/problems/longest-common-subsequence/solution/zui-chang-gong-gong-zi-xu-lie-by-leetcod-y7u0/) 官方题解给出的动态规划方法的解答：

    PS：官方题解中的二位矩阵就是为了遍历所有可能出现的情况，而每一次矩阵单位值的更新，都是比较了上一步所有可能结果的最优值更新的。
...
- 代码：
    ...
    ```javaScript
    let dp = new Array(text1.length + 1).fill(0).map(() => new Array(text2.length + 1).fill(0)) // 初始化二维数组
    for(let i = 1;i<text1.length+1;i++){
        for(let x = 1;x<text2.length +1;x++){
            if(text1[i-1] === text2[x-1]){
                dp[i][x] = dp[i-1][x-1] + 1
            }else{
...
- 复杂度分析：
    ...
    时间复杂度：`O(mn)`，其中 m 和 n 分别是字符串 `text1` 和 `text2` 的长度，空间复杂度也为 `O(n)`。

    [![c2zgI0.png](https://z3.ax1x.com/2021/04/15/c2zgI0.png)](https://imgtu.com/i/c2zgI0)
